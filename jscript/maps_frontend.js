...
//######################################################################################################################
	function display_prime(){
		...
		if($("#full_gis").val()==1){
			// для имплементации нового кода функции:
			// gis_background - gb
			$.ajax({
				url: "/ajax/ajax.php",
				type: "GET",
	...
	function display_search_results(node){
		...
		for(i in node){
			...
			results[results.length]= '<li onclick="display_locations('+ i +');" title="Один щелчок - показать на карте. Двойной - открыть страницу" ondblclick="window.location = \'' +  node[i].link + '\'"> ' + img + ' ' + node[i].name.replace("&nbsp;", " ") + '</li>';
		}
