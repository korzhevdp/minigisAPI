...
function init() {
	...
	archLayer = function () {
			return new ymaps.Layer(function (tile, zoom) {
				//alert(['запрашивает: ', zoom, tile[0], tile[1] ].join("\n") + "\n" + ['должно быть: ', zoom, tile[0], dX[zoom] - tile[1] , "разн: ", (dX[zoom] - tile[1] - tile[1]) ].join("\n"));
	...
	// Добавим конструктор слоя в хранилище слоёв под ключом my#arch
	// ymaps.layer.storage.add('my#arch11', archLayer11);
	// и добавим его в хранилище типов карты под ключом 'my#arch'
	...
	map = new ymaps.Map("YMapsID", {
		...
		zoom: 16,//mp.zoom,// Коэффициент масштабирования
	...
	'<img src="/uploads/ico/$[properties.img|nophoto.gif]" alt="мини" id="sm_src_pic">' +
